...
def get_data(lat1, lat2, lng1, lng2, step):
    Measurements = list(Measurement.objects.filter(longitude__gte=lng1 - step, longitude__lte=lng2 + step,
                                                   latitude__gte=lat1 - step, latitude__lte=lat2 + step))
...
def get_griddata(lon, lat, point_grid):
    ...
    new_lat, new_lon = np.meshgrid(np.linspace(lat[0], lat[-1], len(lat)*mul),
                                   np.linspace(lon[0], lon[-1], len(lon)*mul))
    xi = (new_lat, new_lon)
